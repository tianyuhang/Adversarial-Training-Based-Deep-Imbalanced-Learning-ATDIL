...
def PGD(X_P, y_P, targetModel, rho):
    ...
    attack = fb.attacks.L2ProjectedGradientDescentAttack(steps=5)
    
    raw, clipped, is_adv = attack(fmodel, X_P, y_P, epsilons=rho)
    return np.array(raw)
...
def build_target(num_fea, LOSS='binary_crossentropy'):
    """
    Modified neural network structure: Hidden layers=[64 (ReLU), 32 (ReLU)], Dropout=0.1, Batch Normalization.
    """
    model = keras.models.Sequential()
    
    # Layer 1: Dense (64) + BatchNorm + ReLU + Dropout (0.1)
    model.add(keras.layers.Dense(64, input_shape=(num_fea,)))
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.Activation('relu'))
    model.add(keras.layers.Dropout(0.1))
    
    # Layer 2: Dense (32) + BatchNorm + ReLU + Dropout (0.1)
...
def train_GAE():
    for desired_IR in [5,10,15,20,40,60]:
        ...
        for i in range(1):
            ...
            # Target Model Training with Early Stopping
            target.fit(
                X_tr, 
                y_tr_, 
                epochs=200, 
            ...
            selected = np.random.randint(0, len(X_adv), 50)
            ...
            # Final Classifier Training with Early Stopping
            clf.fit(
                GAE_tr, 
                GAE_y_, 
                epochs=200, 
